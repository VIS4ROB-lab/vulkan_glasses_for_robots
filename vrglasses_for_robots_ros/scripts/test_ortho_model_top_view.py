...
def model_limits(obj_file):
    ...
    for name, material in scene.materials.items():# assuming only one material
        ...
        if material.vertex_format == 'T2F_N3F_V3F':
            ...
            # extract all x coordinates (every 5th element starting with 6th)
            xVertices = material.vertices[2::5]

            # extract all y coordinates (every 5th element starting with 7th)
            yVertices = material.vertices[3::5]

            # extract all z coordinates (every 5th element sarting with 8th)
...
def publisher(obj_file):
    ...
    while not rospy.is_shutdown():
        ...
        rate.sleep()
        print('new pub {}'.format(p.header.stamp))
